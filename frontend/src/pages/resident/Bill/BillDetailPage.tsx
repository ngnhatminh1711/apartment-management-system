...
const BillDetailPage = () => {
    ...
    const fetchBillDetail = async () => {
        try {
            ...
            const msg = e?.response?.data?.message || e?.response?.data?.code || e?.message || "Không thể tải chi tiết hóa đơn";
            toast.error(msg);
        } finally {
        }
    };
    ...
    return (
        <main>
            ...
            >
                ...
                <span className="text-sm font-semibold uppercase tracking-wider">Quay về</span>
            </button>
            <div>
                <div className="bg-white rounded-xl p-8 border border-outline shadow-sm mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div>
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <span className="text-slate-500 text-sm font-medium">Hóa đơn: #{bill.id}</span>
                        </div>
                        <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Kỳ thanh toán {bill.billingMonth}</h1>
                        <div className="flex items-center gap-4 mt-2 text-slate-500 text-sm">
                            <div className="flex items-center gap-1">
                                <span className="material-symbols-outlined text-base" data-icon="calendar_month">
                                    calendar_month
                                </span>
                    ...
                    {(bill.remainingAmount ?? 0) > 0 && (
                        <div className="text-right">
                            <p className="text-sm font-medium text-slate-500 mb-1">Tổng cộng cần thanh toán</p>
                            <p className="text-4xl font-black text-blue-600">{formatCurrency(bill.remainingAmount ?? 0)}</p>
                            <button
                                onClick={() => setSepayOpen(true)}
                                className="btn-primary w-full py-3 text-base flex items-center justify-center gap-2 mt-5"
                            >
                                <span>Chuyển khoản ngân hàng (QR)</span>
                                <span className="text-xs opacity-75 ml-1">– Xác nhận tức thì</span>
                            </button>
                        </div>
                    )}
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2 space-y-8">
                        <ServiceTable data={bill.items ?? []} totalAmount={bill.totalAmount} />
                    </div>
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="bg-primary-container/10 border-dashed border-2 border-primary/30 rounded-xl p-6 flex items-center gap-6">
                            <div className="bg-white p-3 rounded-full shadow-sm">
                                <span className="material-symbols-outlined text-primary text-3xl" data-icon="security">
                                    security
                                </span>
                            </div>
                            <div>
                                ...
                                <p className="text-sm text-slate-600">
                                    Hệ thống hỗ trợ đa dạng phương thức thanh toán bảo mật tuyệt đối qua VNPAY, Bank Transfer và
                                    Thẻ tín dụng.
                                </p>
                            </div>
                        </div>
            ...
            <SepayQrModal
                ...
                onSuccess={() => {
                    ...
                    // Reload bill
                    BillService.getBillDetails(bill.id).then(setBill);
                }}
            />
